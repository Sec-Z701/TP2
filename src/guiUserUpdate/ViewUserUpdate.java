...
public class ViewUserUpdate {
    ...
    private static Alert alertEmailInUse = new Alert(AlertType.INFORMATION);
	...
	private ViewUserUpdate() {
        ...
        button_UpdateEmailAddress.setOnAction((_) -> {result = dialogUpdateEmailAddresss.showAndWait();
    		result.ifPresent(enteredEmail -> {
    			// If another account already has the entered email address, do not update it
    			if (theDatabase.getUserName(enteredEmail) != null) {
    				alertEmailInUse.showAndWait();
    				return;
    			}
     ...
     // If an attempt to update the account with an email that belongs to another users account is made, 
     // this alert dialog will tell the user.
        alertEmailInUse.setTitle("Email In Use");
        alertEmailInUse.setHeaderText("The email provided is being used by another account.");
        alertEmailInUse.setContentText("Enter a different email to continue updating account.");
	}
