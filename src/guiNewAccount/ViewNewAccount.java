...
public class ViewNewAccount {
	...
	*/
    ...
    protected static Label label_NewUserCreation = new Label(" User Account Setup");
    ...
    protected static Button button_UserSetup = new Button("Continue");
    ...
    // This alert is used should the account attempting to be updated has been deleted
    protected static Alert alertAccountNotFound = new Alert(AlertType.INFORMATION);
    
    // This alert is used after an existing user has updated their account login
    protected static Alert alertAccountLoginUpdated = new Alert(AlertType.INFORMATION);
    
	// This alert is used should the invitation code reference the email address of an already 
    // created account.
    protected static Alert alertAccountRoleUpdated = new Alert(AlertType.INFORMATION);
	...
	*/
	...
	public static void displayNewAccount(Stage ps, String ic) {
		...
		String username = theDatabase.getUserName(emailAddress);
		
		// Make sure that if we are trying to modify the username/password to an existing account, 
		// the account actually exists
		if (theRole.equals("None") && username == null) {
			alertAccountNotFound.showAndWait();
			return;
		}

		// Make sure the invitation is still valid and hasn't expired
		boolean codeValid = theDatabase.isCodeValid(theInvitationCode);
		if (!codeValid) {
			alertInvitationCodeIsExpired.showAndWait();
			return;
		}
		
		// If the email address tied to the invitation is already associated with an account, 
		// and there is a role to add, update the account to include that role instead of 
		// creating a new account.
		if (username != null && !theRole.equals("None")) {
			// Add role to account
			theDatabase.updateUserRole(username, theRole, "true");
			
			// Delete invitation code
			theDatabase.removeInvitationAfterUse(theInvitationCode);
			
			// Inform user the account has been updated
			alertAccountRoleUpdated.setContentText("The account with the username \"" + 
												username + "\" has been updated with the " + 
												theRole + " role.");
			alertAccountRoleUpdated.showAndWait();
			return;
		}
		...
		theStage.setTitle("CSE 360 Foundation Code: User Account Setup");	
	...
	private ViewNewAccount() {
		...
		alertInvitationCodeIsExpired.setTitle("Expired Invitation Code");
		alertInvitationCodeIsExpired.setHeaderText("The invitation code is expired.");
		alertInvitationCodeIsExpired.setContentText("Request a new invite code to the system.");
		
		// If we are trying to update an account that has been deleted, this alert dialog will tell the user
		alertAccountNotFound.setTitle("Account Not Found");
		alertAccountNotFound.setHeaderText("Your account could not be found.");
		alertAccountNotFound.setContentText("Contact an admin for help creating a new account.");
		
		// If we changed the username/password to an already created account, this alert dialog will tell the user
		alertAccountLoginUpdated.setTitle("Account Updated");
		alertAccountLoginUpdated.setHeaderText("Account Updated");
		
		// If the invitation code added a role to an already created account, this alert dialog will tell the user
		alertAccountRoleUpdated.setTitle("Account Updated");
		alertAccountRoleUpdated.setHeaderText("The invitation code is linked to an existing account.");
